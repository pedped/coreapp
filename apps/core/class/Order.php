...
class Order {
    ...
    public function OnSuccessPayment(&$errors, $orderPayementType, $paymentID) {
        ...
        if (intval($orderid) > 0) {
            $this->OnSuccessOrder($errors, $orderid);
        } else {
...
//        return;
    ...
    public function GetOrderAmountCurrency($orderid) {
        ...
        $productClassName = Text::camelize(UserOrder::findFirst($productTypeID)->key);
        $productable = new $productClassName();
    ...
    public function PayOrder(&$errors, $orderid, $paymentTypeID) {
        ...
        $transactionID = $transaction->StartTransaction($errors, $info->Price, $info->Currency, __TRANSACTIONTYPE_ORDER, $orderid);
        ...
        if (intval($transactionID) > 0) {
            ...
            $this->paymentHandler->StartPayment($errors, $this->userid, $paymentTypeID, $orderid, $transactionID);
        } else {
